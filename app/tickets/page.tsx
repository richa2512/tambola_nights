...
import { TicketGrid, generateTickets, Ticket } from "@/lib/ticket-generator";
import { generateFullSheetTickets } from "@/lib/sheet-generator";
import { TicketCard } from "@/components/TicketCard";
import { TicketSheet } from "@/components/TicketSheet";
import { useGameStore, SessionConfig } from "@/store/useStore";
import Papa from "papaparse";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { Download, Upload, Plus, Trash2, Home, Copy, Settings } from "lucide-react";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <form 
          ...
          className="glass p-8 rounded-3xl shadow-xl w-full max-w-lg border border-border"
        >
          <div className="flex items-center gap-3 mb-6">
            <Settings className="w-8 h-8 text-primary-500" />
            <h1 className="text-3xl font-bold">Session Setup</h1>
          </div>
          ...
          <button type="submit" className="w-full py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold text-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass p-6 rounded-2xl shadow-sm border border-border">
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white p-3 rounded-lg font-medium transition-colors"
            >
        ...
        <div className="glass p-6 rounded-2xl shadow-sm border border-border">
          ...
          <p className="text-sm text-slate-500 mb-4">Upload a CSV file with columns: `Player Name` and `Number of tickets`.</p>
            <label className="w-full flex flex-col items-center justify-center border-2 border-dashed border-border rounded-lg p-6 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
          ...
          {tickets.length > 0 && (
            <div className="glass p-6 rounded-2xl shadow-sm border border-border space-y-4">
              <h2 className="text-xl font-semibold">Actions</h2>
              <button 
                onClick={exportPDF}
                disabled={isExporting}
                className="w-full flex items-center justify-center gap-2 bg-slate-800 text-white hover:bg-slate-900 p-3 rounded-lg font-medium transition-colors disabled:opacity-50"
              >
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 border border-red-200 text-red-600 hover:bg-red-50 p-3 rounded-lg font-medium transition-colors dark:border-red-900 dark:hover:bg-red-900/20"
              >
                <Trash2 className="w-5 h-5" /> Clear All Tickets
              </button>
              <button 
                onClick={endSession}
                className="w-full flex items-center justify-center gap-2 border border-orange-200 text-orange-600 hover:bg-orange-50 p-3 rounded-lg font-medium transition-colors dark:border-orange-900 dark:hover:bg-orange-900/20"
              >
                End Current Session
              </button>
            </div>
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-semibold">Generated Tickets</h2>
                  ...
                  >
                    ...
                    >
                      <div className="font-bold text-lg flex items-center gap-3">
                        <span className="text-slate-800 dark:text-slate-200">{playerId}</span>
                      </div>
                    ...
                    {expandedPlayer === playerId && (
                      <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/20 flex flex-col items-center overflow-x-auto w-full">
                        <TicketSheet 
